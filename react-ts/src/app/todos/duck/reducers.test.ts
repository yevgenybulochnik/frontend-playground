...
describe('TodoApp set visibility filter', () => {
  it('should have a inital state of SHOW_ALL', () => {
    ...
    const result = todoReducer.todoApp(state, action)
    expect(result.visibilityFilter).to.equal('SHOW_ALL')
  })
  it('should change filter on action', () => {
    let state = undefined
    const result = todoReducer.todoApp(state, todoActions.setVisibilityFilter('SHOW_COMPLETED'))
  ...
  it('should be able to toggle todo', () => {
    const todoState = todoReducer.todoApp(undefined, todoActions.addTodo('Working on stuff'))
    let action = todoActions.toggleTodo(0)
    const result = todoReducer.todoApp(todoState, action)
